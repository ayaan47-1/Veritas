...
import { getCurrentUser, getRisks, reviewRisk } from "@/lib/api";
import type { CurrentUser, ReviewDecision, Risk } from "@/lib/types";
...
export default function RisksClientPage() {
  ...
  const loadPage = useCallback(
    async (cursor: string | number, append: boolean) => {
      ...
      try {
        const [currentUser, risks] = await Promise.all([
          getCurrentUser(getToken),
          getRisks(getToken, { assetId, cursor, limit: 20 }),
        ]);
        setUser(currentUser);
        setItems((prev) => (append ? [...prev, ...risks.items] : risks.items));
        setNextCursor(risks.next_cursor);
      } catch (loadError) {
        const message = loadError instanceof Error ? loadError.message : "Failed to load risks";
        setError(message);
      } finally {
        setIsLoading(false);
      }
  ...
  async function submitReview(payload: {
    ...
    const response = await reviewRisk(getToken, reviewTarget.id, {
      ...payload,
      reviewer_id: user.id,
    });
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-semibold text-slate-900">Risks Table</h1>
          ...
          <div className="flex gap-2">
            ...
            <Link
              href={assetId ? `/obligations?asset_id=${assetId}` : "/obligations"}
        ...
        {isLoading ? <p className="text-sm text-slate-600">Loading risks...</p> : null}
