...
function buildRiskScoreBreakdown(risk: RiskDetail): ScoreBreakdownItem[] {
  ...
  if (STATUTE_RE.test(risk.risk_text)) items.push({ label: "Statute or regulation reference detected", delta: 10 });
  if (MONETARY_RE.test(risk.risk_text)) items.push({ label: "Monetary amount detected", delta: 5 });
  if (DEADLINE_RE.test(risk.risk_text)) items.push({ label: "Explicit deadline language", delta: 5 });
  if (risk.contradiction_flag) items.push({ label: "Linked to contradicting obligation", delta: 5 });
  if (risk.has_external_reference) items.push({ label: "External reference present", delta: 5 });
  if (risk.evidence.some((ev) => ev.source === "ocr")) items.push({ label: "Evidence came from OCR text", delta: -15 });
  if (risk.contradiction_flag) items.push({ label: "Contradiction detected", delta: -30 });
  return items;
}
